    ...
    loop {
        ...
        for ev in season.events {
            ...
            let db_event: Event = match sqlx::query_as_unchecked!(
                Event,
                "SELECT `id` as `id?`, name, year, created, series FROM events where name = ? AND year = ?",
                ev.title,
                year
            )
                .fetch_optional(pool)
                ...
                Ok(None) => {
                    match insert_event(pool, year, &ev).await {
                        Err(why) => {
            ...
            for (i, doc) in ev.documents.iter().enumerate() {
                ...
                let mirror_url =
                    match upload_mirror(title, &db_event.name, year, &body)
                        .await
                    {
                        Err(why) => {
